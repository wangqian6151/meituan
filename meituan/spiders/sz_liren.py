...
from meituan.share import html_from_uri, calc_md5, timestamp_to_mytime, html_from_uri_first
...
class SzLirenSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        raw_data = html_from_uri_first(response.url)
        # raw_data = response.text
        pprint('raw_data:{}'.format(raw_data))
        self.logger.debug('raw_data:{}'.format(raw_data))
        ...
        if json_data:
            ...
            # 解析商圈地址结束，由于dict_data.get('area').get('id')代表不选择商圈(即默认选择全部)，此时的json数据中deals字段（即商品数量不全）所以放在最后。
        ...
        self.logger.debug('area_id_list:{} len: {}'.format(self.area_id_list, len(self.area_id_list)))
        print('category_id_list:{} len: {}'.format(self.category_id_list, len(self.category_id_list)))
        self.logger.debug('category_id_list:{} len: {}'.format(self.category_id_list, len(self.category_id_list)))
    ...
    def parse_shop_detail(self, response):
        ...
        self.logger.debug('parse_shop_detail response.text:{}'.format(response.text))
        ...
        if json_data:
            ...
            self.logger.debug('pjson_data:{}'.format(json_data))
            ...
            for shop in shop_list:
                ...
                pprint('11111111111111111111111111111111111111111:{}'.format(json_data))
                self.logger.debug('111111111111111111111111111111111111:{}'.format(json_data))
                # shop_item['categoryIdList'] = ', '.join(cate for cate in shop.get('cate')) if shop.get('cate') else None
                shop_item['categoryIdList'] = str(shop.get('cate'))
                ...
                if deals:
                    ...
                    for deal in deals:
                        pprint('222222222222222222222222222222222:{}'.format(json_data))
                        self.logger.debug('222222222222222222222222222:{}'.format(json_data))
                        ...
                        yield scrapy.Request(goods_comment_url, callback=self.parse_goods_comment,
                                             meta={'dealid': dealid, 'offset': 0, 'page': 1})
                        pprint('parse_shop_detail shopId:{}, dealsId:{}'.format(goods_item['shopId'], goods_item['id']))
                        self.logger.debug('parse_shop_detail shopId:{}, dealsId:{}'.format(goods_item['shopId'], goods_item['id']))
                        yield goods_item
                ...
                yield scrapy.Request(shop_comment_url, callback=self.parse_shop_comment,
                                     meta={'shopid': shopid, 'offset': 0, 'page': 1})
                true_total_count += 1
